  ...
  try {
    ...
    if (myUser) {
      ...
      if (otherUser) {
        ...
        if (otherUser.follower_handle_names.includes(myUser.handle_name)) {
          isFollowing = true
        }
        ...
        const resData = {
          ...
          followee_shops_handle_names: otherUser.followee_shops_handle_names,
        ...
        if (shop) {
          if (!shop.publish_state) {
            user.followee_shops_handle_names.splice(
              user.followee_shops_handle_names.indexOf(shop.handle_name),
              1
            )
          }
        }
